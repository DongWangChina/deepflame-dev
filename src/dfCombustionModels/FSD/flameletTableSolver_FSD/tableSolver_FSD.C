...
tableSolver_FSD::tableSolver_FSD(word tablePath)
:
small(1.0e-4),
smaller(1.0e-8),
smallest(1.0e-12),
...
H_ox("H_ox",dimensionSet(0,2,-2,0,0,0,0),Hox)
{
    ...
    if (table.is_open())
    {
        ...
        I0_Tab = { new double[NZK*NK]{} };
        Info << "Reading I0 table\n" <<endl;
        count = 0;
        for (int ii = 0; ii < NZK; ++ii)
        {   
            if (std::getline(table, line))
...
tableSolver_FSD::~tableSolver_FSD()
{
    if (Pstream::parRun())
    {
        forAll(wins_, tableI)
...
//- solve variance 
double Foam::tableSolver_FSD::cal_gvar
(
    double mean, 
    double var, 
    double Ycmax
...
int Foam::tableSolver_FSD::locate_higher
(
    int n, 
    double array[], 
    double x
...
double Foam::tableSolver_FSD::intfac
(
     double xx, 
     double low, 
     double high
...
double Foam::tableSolver_FSD::intfac_high
(
     double xx,
     double high, 
     double low
...
double Foam::tableSolver_FSD::interp1d
(
  int n1, 
  int loc_z,
  double zfac, 
...
double Foam::tableSolver_FSD::interp2d
(
...
double Foam::tableSolver_FSD::interp3d
(
...
double Foam::tableSolver_FSD::lookup2d
(
...
double Foam::tableSolver_FSD::lookup3d
(
    int n1, double list_1[], double x1,
    int n2, double list_2[], double x2,
...
double Foam::tableSolver_FSD::sdrLRXmodel
(
